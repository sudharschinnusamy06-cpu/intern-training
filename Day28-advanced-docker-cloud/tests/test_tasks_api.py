def get_auth_token(client, username="testuser"):
    client.post("/v1/register", params={"username": username, "password": "testpass123"})
    response = client.post("/v1/login", params={"username": username, "password": "testpass123"})
    return response.json()["access_token"]


def test_create_task(client):
    token = get_auth_token(client, "user1")
...
def test_get_all_tasks(client):
    token = get_auth_token(client, "user2")
    client.post(
        "/v1/tasks",
...
def test_get_single_task(client):
    token = get_auth_token(client, "user3")
...
def test_update_task(client):
    token = get_auth_token(client, "user4")
    create_response = client.post(
        "/v1/tasks",
...
def test_delete_task(client):
    token = get_auth_token(client, "user5")
    create_response = client.post(
        "/v1/tasks",
...
def test_update_task_not_found(client):
    token = get_auth_token(client, "user6")
    response = client.put(
        "/v1/tasks/9999",
...
def test_delete_task_not_found(client):
    token = get_auth_token(client, "user7")
    response = client.delete(
        "/v1/tasks/9999",
        headers={"authorization": f"Bearer {token}"},
    )
...
def test_pagination_limit(client):
    token = get_auth_token(client, "user8")
    client.post(
        "/v1/tasks",
...
def test_filter_completed(client):
    token = get_auth_token(client, "user9")
    client.post(
        "/v1/tasks",
...
def test_register_user(client):
    response = client.post(
        "/v1/register",
        params={"username": "regtest", "password": "testpass123"},
    )
    assert response.status_code == 200
...
def test_login_success(client):
    client.post("/v1/register", params={"username": "loginuser", "password": "testpass123"})

    response = client.post(
        "/v1/login",
        params={"username": "loginuser", "password": "testpass123"},
    )
...
def test_login_wrong_password(client):
    client.post("/v1/register", params={"username": "wronguser", "password": "correctpass"})

    response = client.post(
        "/v1/login",
        params={"username": "wronguser", "password": "wrongpass"},
    )
